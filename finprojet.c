...
Produit catalogue[NB_PRODUITS] = {
    ...
    {8, "Disque Dur Externe", "Accessoires", 99.0, 10, "1 To USB 3.0"},
    {9, "Webcam", "Accessoires", 59.0, 7, "Webcam HD 1080p"},
    {10, "Chargeur USB", "Accessoires", 19.0, 25, "Charge rapide 20W"},
};
...
void tri_produits(){
    ...
    for (int i = 0; i < n-1; i++)
    {
        ...
        else if (ch == 3 && strcmp(catalogue[i].prix > catalogue[j].prix)>0)
        condition = 1;
                 
             if (condition)
             {
              catalogue[i].prix=catalogue[j].prix
    ...
    printf("produit tries avec succes\n");
}
...
switch (choix ){
    ...
    break;
    case 0 :
   printf("D.Youssef vous salue. Au revoir\n");
    break;
